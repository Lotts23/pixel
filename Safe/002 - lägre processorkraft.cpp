...
private:
    ...
    void createRulerPatterns() {
        sf::Image rulerPatternImage;
        rulerPatternImage.create(128, 64, sf::Color::White);

        for (int y = 0; y < 64; ++y) {
            for (int x = 0; x < 128; ++x) {
                if ((x / 4 + y / 4) % 2 == 1) {
                    rulerPatternImage.setPixel(x, y, sf::Color::Black);
                }
            }
        }

        rulerPatternTexture.loadFromImage(rulerPatternImage);

        topRulerPattern.setTexture(&rulerPatternTexture);
        topRulerPattern.setScale(4.0f, 4.0f);
        topRulerPattern.setPosition(24, 1);

        leftRulerPattern.setTexture(&rulerPatternTexture);
        leftRulerPattern.setScale(4.0f, 4.0f);
        leftRulerPattern.setPosition(0, 24);
    }

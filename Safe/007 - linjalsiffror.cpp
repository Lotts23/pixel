...
class DrawingApp {
public:
    DrawingApp(int initialSizeX = 560, int initialSizeY = 300) : sizeX(initialSizeX), sizeY(initialSizeY), window(sf::VideoMode(initialSizeX, initialSizeY), "Ritprogram") {
...
private:
    ...
    void createText() {
        ...
        for (int i = 0; i <= 256; i += 8) {
            ...
            scaleText.setPosition(1, 26 + (i*4) - 8);
            leftRulerTexts.push_back(scaleText);

            scaleText.setPosition(26 + (i*4) - 8, 1);
    ...
    void createSquareShapes() {
        for (int i = 0; i < numSquares; ++i) {
            ...
            squareShapes[i].setPosition(20, 25 + (i * (squareSize + 1)));
        }
    }
    ...
    void createTopSquareShapes() {
        for (int i = 0; i < numSquaresTop; ++i) {
            ...
            squareShapesTop[i].setPosition(25 + (i * (squareSize + 1)), 20);
        }
    }

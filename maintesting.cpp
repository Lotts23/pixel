...
private:
    ...
    std::vector<VertexArray> tempShiftLines;
    ...
    if (isShiftKeyPressed) {
        ...
        if (startPixelX != endPixelX || startPixelY != endPixelY) {
            ...
            for (float t = 0.0f; t <= 1.0f; t += 1.0f / steps) {
                ...
                quad[1].position = sf::Vector2f(x + scale, y);
                quad[2].position = sf::Vector2f(x + scale, y + scale);
                quad[3].position = sf::Vector2f(x, y + scale);
                for (int i = 0; i < 4; ++i) {
                    quad[i].color = sf::Color::Red; 
                }
                tempShiftLines.append(quad);
            }
        }
...
/* 
        if (isDrawing && isShiftKeyPressed) {
            ...
            int endPixelX = static_cast<int>((lineEnd.x  + 25 + 20) / scale);
            int endPixelY = static_cast<int>((lineEnd.y + 25) / scale);

            

            while (startX != endPixelX || startY != endPixelY) {
                ...
                if (startX < endPixelX) {
                    startX += scale;
                } else if (startX > endPixelX) {
                    startX -= scale;
                }
                if (startY < endPixelY) {
                    startY += scale;
                } else if (startY > endPixelY) {
                    startY -= scale;
                }
            }
        }
